# Assume  Anaconda or Miniconda is installed 
#1 jupyter nbconvert --to python lecture1.ipynb =>= Generate lecture1.py
#2 jpynb to .py by "conda install -c conda-forge jupytext" followed by
#              jupytext --to notebook lecture1.py  =>=lecture1.ipynb
# CMIT 2026 Python Lecture 1 == CMIT_P1.ipynb 
# 22 June 2025 9am      (If for PY, jupyter nbconvert --to script Lecture_1.ipynb)
...
name = a.split("\\")[-1]
print(f"{name} \t\t\t| b={b:04d} \t | c={c:.4f}")    # format new
import os, cv2, matplotlib.pyplot as plt
my_image = 'Liver.jpg';  my_imag2 = 'Glasgow.jpg'
if os.path.exists( my_image ):
    A  = cv2.imread(my_image, 1)    # default colour (n,n,3) BGR
    A0 = cv2.imread(my_image, 0)    # no colour so only (n,n)
    A = cv2.cvtColor(A, cv2.COLOR_BGR2RGB)
    B  = cv2.imread(my_imag2, 1)    # default colour (n,n,3) BGR
    B0 = cv2.imread(my_imag2, 0)    # no colour so only (n,n)
    B = cv2.cvtColor(B, cv2.COLOR_BGR2RGB)
fig = plt.figure(figsize=(12, 12)) 
ax1 = fig.add_subplot(2, 2, 1)     # Left subplot (similar to MATLAB)
ax1.imshow(A); ax1.set_title('Colour Image') 
ax2 = fig.add_subplot(2, 2, 2)     # Right subplot
ax2.imshow(A0, cmap='gray') # cmap is optional as already (n,n)
ax2.set_title('Gray Image'); ax2.axis('off')
ax3 = fig.add_subplot(2, 2, 3)     # Left subplot (similar to MATLAB)
ax3.imshow(B); ax3.set_title('Colour Image') 
ax4 = fig.add_subplot(2, 2, 4)     # Right subplot
ax4.imshow(B0, cmap='gray') # cmap is optional as already (n,n)
ax4.set_title('Gray Image'); ax4.axis('off')
print('Images plotted -  to proceed, close it (if in plain py)');  plt.show()
...
s=r'TASK:  If not sure about the maths concepts, check wiki or other sources.\n\
